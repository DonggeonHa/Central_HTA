 ...
 * 			Student getStduentByName(String name)
 * 		- Update
...
public class StudentRepository {
	...
	private void loadData() {
		try (FileReader fileReader = new FileReader("src/day1/students.txt");
			...
			while ((text=reader.readLine()) != null) {
				...
				// Student <-- name : "김유신", kor : 100, eng : 100, math : 100, total : 300, average : 100;
				Student student = new Student(name, kor, eng, math);
				
				// 4. 학생성적정보가 들어있는 Student객체를 students 참조변수가 참조하는 
				//    ArrayList 객체에 추가한다.
				// 블록내 코드가 한번 실행될 때마다 students가 참조하는 ArrayList객체에 Student객체 주소값이 계속 저장된다.
				// students <-- [Student객체 주소값]
				// students <-- [Student객체 주소값, Student객체 주소값]
				// students <-- [Student객체 주소값, Student객체 주소값, Student객체 주소값]
				// students <-- [Student객체 주소값, Student객체 주소값, Student객체 주소값, Student객체 주소값]
				// students <-- [Student객체 주소값, Student객체 주소값, Student객체 주소값, Student객체 주소값, Student객체 주소값]
	...
	public Student getStudentByName(String name) {
		...
		// 이름이 일치하는 학생정보가 존재하지 않으면 null이 반환한다.
		return null;
	}
	
	// 지정된 이름의 학생정보를 삭제한다.
	public void removeStudentByName(String name) {
		Iterator<Student> iter = students.iterator();	// 해당 ArrayList에 대한 반복자 객체가 획득함.
		while (iter.hasNext()) {
