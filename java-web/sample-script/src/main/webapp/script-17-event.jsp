...
<html lang="ko">
	...
	<body>
		<div class="container">
			<div class="row">
				<div class="col-12">
					<form action="#" method="post" class="border p-3 bg-light" id="register-form">
		...
		<script type="text/javascript">
			function checkForm() {
				...
				if (!/^[a-zA-Z0-9]+$/.test(userIdEl.value)) {
				...
				// 경력사항을 선택했는지 체크
				var careerEl = document.querySelector("select[name='career']"); // 검색된 엘리먼트 중 첫번째 엘리먼트
				if (!careerEl.value) {
					alert("경력사항을 선택 하세요");
					careerEl.focus();
					return;
				...
				if (!etcEl.value) {
					var confirmVAlue = confirm("기타사항에 입력된 내용이 하나도 없는데, 괜찮아?");
					if (!confirmvalue) {
						etcEl.focus();
						return;
					}
