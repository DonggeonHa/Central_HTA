					...
					$.getJSON(url, {key:'f5eef3421c602c6cb7ea224104795888', targetDt:dateString})
						.done(function(data) {
							...
							$.each(boxofficeList, function(index, boxoffice) {
								var $tr = $('<tr></tr>');
								$('<td></td>').text(boxoffice.rank).appendTo($tr);
								$('<td></td>').text(boxoffice.movieNm).appendTo($tr);
								$('<td></td>').text(boxoffice.openDt).appendTo($tr);
								$('<td></td>').text(boxoffice.salesShare).appendTo($tr);
