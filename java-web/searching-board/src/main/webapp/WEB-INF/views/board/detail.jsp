...
<div class="container">
	...
	<main class="mb-3">
		...
		<div class="row mb-2">
			<div class="col-6">
				<c:url var="modifyURL" value="modify">
					<c:param name="no">${param.no }</c:param>
					<c:param name="page">${param.page }</c:param>
					<c:if test="${not empty param.opt }">
						<c:param name="opt">${param.opt }</c:param>
					</c:if>
				...
				<a href="${modifyURL }" class="btn btn-outline-warning">수정</a>
				<a href="${deleteURL }" class="btn btn-outline-danger">삭제</a>
			</div>
